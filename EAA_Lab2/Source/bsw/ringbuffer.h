...
/* Write a single byte */
inline RC_t Ringbuffer_Put(ringbuffer_hdl_t* rb, uint8_t data)
{
    if (rb->filled < rb->size)
    {
        ((uint8_t*)rb->p_payload)[rb->idx_write++] = data;
        rb->filled++;
        rb->idx_write %= rb->size;
...
/* Get a single byte */
inline RC_t Ringbuffer_Get(ringbuffer_hdl_t* rb, uint8_t* data)
{
    if (rb->filled > 0)
    {
        *data = ((uint8_t*)rb->p_payload)[rb->idx_read++];
        rb->filled--;
        rb->idx_read %= rb->size;
...
inline RC_t Ringbuffer_Clear(ringbuffer_hdl_t* rb)
{
    rb->idx_read=0;
    rb->idx_write=0;
    rb->filled=0;
